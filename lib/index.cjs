...
const fileToRoutes = (files, appData, hasLayout) => {
  ...
  return files.reduce(
    (acc, file) => {
      ...
      const componentPath = path__default.relative(appData.absRouterPath, file);
...
class WebpackPluginAutoRoutes {
  ...
  // 获取数据
  getAppData({ cwd }) {
    const resolveAppPath = (relativePath, base = cwd) => path__default.resolve(base, relativePath);
    ...
    if (this.entry) {
      if (path__default.isAbsolute(this.entry)) {
  ...
  generate(appData, isMetaChange = false) {
    const routerComponentPaths = [
      path__default.resolve(appData.absRouterPath, "index.tsx"),
      path__default.resolve(appData.absRouterPath, "index.jsx"),
      path__default.resolve(appData.absRouterPath, "index.js"),
      path__default.resolve(appData.absRouterPath, "index.ts")
    ];
